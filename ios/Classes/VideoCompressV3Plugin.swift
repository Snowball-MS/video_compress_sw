...
public class VideoCompressSWPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let instance = VideoCompressSWPlugin(channel: channel)
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "getByteThumbnail":
            ...
            getByteThumbnail(path, quality, position, maxWidth, maxHeight, result)
        case "getFileThumbnail":
            ...
            let maxWidth = args!["maxWidth"] as! NSNumber
            let maxHeight = args!["maxHeight"] as! NSNumber
            getFileThumbnail(path, quality, position, maxWidth, maxHeight, result)
        case "getMediaInfo":
            let path = args!["path"] as! String
    ...
    private func getBitMap(_ path: String,_ quality: NSNumber,_ position: NSNumber,_ maxWidth: NSNumber,_ maxHeight: NSNumber,_ result: FlutterResult)-> Data?  {
        ...
        if maxWidth.intValue == 0 && maxHeight.intValue == 0 {
            assetImgGenerate.maximumSize = CGSize(width: track.naturalSize.width, height: track.naturalSize.height)
        } else {
            assetImgGenerate.maximumSize = CGSize(width: CGFloat(truncating: maxWidth), height: CGFloat(truncating: maxHeight))
        }
...
//        assetImgGenerate.requestedTimeToleranceBefore = .zero
    ...
    private func getByteThumbnail(_ path: String,_ quality: NSNumber,_ position: NSNumber,_ maxWidth: NSNumber,_ maxHeight: NSNumber,_ result: FlutterResult) {
        if let bitmap = getBitMap(path,quality,position,maxWidth,maxHeight,result) {
            result(bitmap)
        }
    }

    private func getFileThumbnail(_ path: String,_ quality: NSNumber,_ position: NSNumber,_ maxWidth: NSNumber,_ maxHeight: NSNumber,_ result: FlutterResult) {
